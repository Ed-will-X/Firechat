...
class GetChatImageUseCase @Inject constructor(
    val repository: ChatImageRepository
) {
    suspend operator fun invoke(imageID: String, chatRoomId: String): Flow<Resource<Image>> {
        return repository.getChatImage(imageID, chatRoomId)
    }
}