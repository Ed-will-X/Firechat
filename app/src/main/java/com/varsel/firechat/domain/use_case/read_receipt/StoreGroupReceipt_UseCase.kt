...
class StoreGroupReceipt_UseCase @Inject constructor(
    val repository: MessageRepository,
    val checkReceipt: HasBeenRead_UseCase
){
    operator fun invoke(message: Message, groupRoom: GroupRoom): Flow<Response> {
        val exists = checkReceipt(groupRoom, message)
        if(!exists) {
//            Log.d("RECEIPT", "Doesn't exist")
//            Log.d("RECEIPT", "Read By: ${message.readBy.keys}")
            return repository.storeReceipt_groupRoom(message, groupRoom.roomUID)
        }
